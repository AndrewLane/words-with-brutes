    ...
    internal class DeterminePotentialPlayLocationsForNextWord : IDeterminePotentialPlayLocationsForNextWord
    {
        ...
        public IEnumerable<PotentialPlayLocation> GetPlacesToPlayNextWord(GameState gameState)
        {
            ...
            return GetPlacesToPlayNextWord(_gameStateIntoTwoDimensionalArrayTransformer.TransformIntoBoolMultiArray(gameState), maxLettersThatCanBeUsed,
                gameState.Challenge.BoardConfiguration.StartingTileLocation);
        }
        ...
        /// </summary>
        private static IEnumerable<PotentialPlayLocation> GetPlacesToPlayNextWord(bool[,] occupiedTiles, int maxLettersThatCanBeUsed, TileLocation startingTileLocation)
        {
            ...
            for (int x = 0; x < occupiedTiles.GetLength(dimension: 0); x++)
            {
                for (int y = 0; y < occupiedTiles.GetLength(dimension: 1); y++)
                {
                    ...
                    {
                        finalResults.AddRange(GetPlacesToPlayNextWord(occupiedTiles, maxLettersThatCanBeUsed, new TileLocation { X = x, Y = y }, startingTileLocation));
                    }
                }
            }
            return finalResults;
        ...
        /// </summary>
        private static IEnumerable<PotentialPlayLocation> GetPlacesToPlayNextWord(bool[,] occupiedTiles, int maxLettersThatCanBeUsed, TileLocation topLeftTileLocation, TileLocation startingTileLocation)
        {
            ...
            {
                var possiblePlayHorizontally = GetPlacesToPlayNextWord(occupiedTiles, topLeftTileLocation, i, WordDirection.Horizontal, startingTileLocation);
                ...
                var possiblePlayVertically = GetPlacesToPlayNextWord(occupiedTiles, topLeftTileLocation, i, WordDirection.Vertical, startingTileLocation);
        ...
        private static PotentialPlayLocation GetPlacesToPlayNextWord(bool[,] occupiedTiles, TileLocation topLeftTileLocation, int numberOfTilesToUse,
            WordDirection wordDirection, TileLocation startingTileLocation)
        {
            ...
            bool playedOnStartingTile = false;
            while (tilesPlayed < numberOfTilesToUse)
            {
                ...
                if (currentTileLocation.X == startingTileLocation.X && currentTileLocation.Y == startingTileLocation.Y)
                {
                    playedOnStartingTile = true;
                }
            ...
            //if we had to skip over a tile or we played on the starting location, we know we're good to go
            if (skippedOverAnExistingTile || playedOnStartingTile)
            {
